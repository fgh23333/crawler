...
// 启动时预热：用所有学号登录一遍，验证可用性，仅保留成功的进入轮换池
...
async function singleRequest(subject, headers) {
    ...
    // 检测 sid 过期（code!=200 或无 paperStore，且提示登录/过期/401）
    const body = res.data;
...
async function fetchAll() {
    ensureDir(DIR.raw);
    await warmupLogin(); // 先登录所有学号预热 sid 缓存
