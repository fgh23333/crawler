...
function fetchData(item, callback) {
    ...
    fs.readFile(__dirname + `/new3/${subjectName}.json`, 'utf8', async (err, data) => {
        if (err) {
            console.error(err);
            callback(err);
        } else {
...
function writeFile(subjectName, jsonArray) {
    fs.writeFile(__dirname + `/new3/solved/${subjectName}Solved.json`, JSON.stringify(jsonArray), (err) => {
        if (err) {
            console.log(err);
        } else {
            console.log('成功写入文件');
